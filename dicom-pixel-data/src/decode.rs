...
use crate::decode::DecodeError::UnsupportedNumberOfBits;
...
/// Decodes the given data based on a set of decoding parameters.
///
/// # Arguments
///
...
/// # Returns
///
/// Returns a Result containing the decoded pixel values on success, or a DecodeError if decoding failed.
pub fn decode(data: &DecodingData) -> Result<PixelValues, DecodeError> {
    match data.transfer_syntax.as_str() {
        uids::EXPLICIT_VR_LITTLE_ENDIAN | uids::IMPLICIT_VR_LITTLE_ENDIAN => { decode_little_endian(data) }
        &_ => {
            Err(DecodeError::UnsupportedTransferSyntax(data.transfer_syntax.clone()))
        }
    }
}
